...
void loop() {
  ...
  Serial.println("Switch state is " + String(switchState) + " is turned on: " + String(isSwitchTurnedOn));

  if (isSwitchTurnedOn) {
    Serial.println("Switch it on, turning on led and run");
    led.turnOn();
    run();
    isLidOpen = false;
    led.turnOff();
  } else {
    Serial.println("Switch is not on, checking proximity sensor");
    // Check the proximity sensor.
    if (sensor.isInRange()) {
      Serial.println(" yes sensor is in range");
      ...
      Serial.println(" no sensor not in range");
      if (isLidOpen) {
        Serial.println("  lid was open, closing fast");
        closeLidFast();
        isLidOpen = false;
      }
    }
